...
describe("lottery", () => {
  ...
  it("Winner withdraws funds", async () => {
    ...
    let lotteryBalanceBefore: number = await provider.connection.getBalance(lottery.publicKey);
    console.log(`Total SOL in lottery account before payout: ${lotteryBalanceBefore}`);
    ...
    console.log("lottery gains {}", endBalanace - startBalance );
    
    expect(endBalanace - startBalance).to.be.lessThan(lotteryBalanceBefore * 91 / 100);
    expect(endBalanace).to.be.greaterThan(startBalance);

    let lotteryBalanceAfter: number = await provider.connection.getBalance(lottery.publicKey);

    console.log("lot balance after {}", lotteryBalanceAfter);
  });
  ...
  it("admin withdraws funds", async () => {
    ...
    // Assert winner got the payout
    let endBalanace = await provider.connection.getBalance(lottery_admin.publicKey);
    console.log("lottery remains for admin {}", endBalanace);
    ...
    expect(lotteryBalanceAfter).to.equal(0);
  });
