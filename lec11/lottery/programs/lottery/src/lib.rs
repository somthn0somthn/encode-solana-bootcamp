...
declare_id!("3bBWZ3FEcsVVJ6wxwNMzy3BYSjPkx35ZvG6WZmfV2NAr");
...
mod lottery {
    ...
    pub fn pay_out_winner(ctx: Context<Payout>) -> Result<()> {
        ...
        msg!("Total lottery balance is {}", balance);

        let partial_balance: u64 = balance.checked_mul(9)
            .ok_or_else(|| ErrorCode::ArithmeticOverflow)?
            .checked_div(10)
            .ok_or_else(|| ErrorCode::ArithmeticOverflow)?;

        msg!("90% of lottery balance is {}", partial_balance);
            
        **lottery.to_account_info().try_borrow_mut_lamports()? -= partial_balance;
        **recipient.to_account_info().try_borrow_mut_lamports()? += partial_balance; 
        
        Ok(())
    }

    pub fn pay_out_admin(ctx: Context<PayoutAdmin>) -> Result<()> {
        ...
        let remaining_balance: u64 = lottery.to_account_info().lamports();   

        msg!("remaining lottery balance is {}", remaining_balance);
            
        **lottery.to_account_info().try_borrow_mut_lamports()? -= remaining_balance;
        **admin.to_account_info().try_borrow_mut_lamports()? += remaining_balance; 
...
pub struct Create<'info> {
    #[account(init, payer = admin, space = 8 + 280)]
...
#[derive(Accounts)]
pub struct PayoutAdmin<'info> {
    #[account(mut,
        constraint =
        lottery.authority == *admin.key
    )]
    pub lottery: Account<'info, Lottery>,
    #[account(mut)]
    pub admin: Signer<'info>,
